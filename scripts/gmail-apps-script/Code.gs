...
function ingestMotoSnapReports() {
  ...
  try {
    ...
    if (!endpoint || !assessmentEndpoint || !secret) throw new Error('Set COMMSIQ_INGEST_URL, COMMSIQ_ASSESS_URL, and COMMSIQ_INGEST_SECRET in Script Properties.');
    ...
    for (const thread of threads) {
      for (const message of thread.getMessages()) {
        ...
        if (processed.has(messageId) || !isMotoSnapReport_(message)) continue;

        const attachments = message.getAttachments({ includeInlineImages: false, includeAttachments: true }).filter(blob => /\.xlsx$/i.test(blob.getName()));
        ...
        for (const attachment of attachments) {
          const response = UrlFetchApp.fetch(endpoint, {
            method: 'post',
            headers: { 'x-commsiq-ingest-secret': secret, 'x-gmail-message-id': messageId },
          ...
          let ingestResult;
          try { ingestResult = JSON.parse(response.getContentText()); }
          catch (error) {
          ...
          if (!batchIds.length) {
            console.error(`CommsIQ ingest returned no batch IDs for ${messageId}.`);
            messageSucceeded = false;
            break;
          }
          ...
          for (const batchId of batchIds) {
            ...
            assessmentBodies.push(assessmentResult.body);
          }
          if (!messageSucceeded) break;

          console.log(`CommsIQ processed ${messageId}: ${response.getContentText()} assessments=${assessmentBodies.join(' | ')}`);
        }
...
function runAssessmentToCompletion_(endpoint, secret, batchId, messageId) {
  ...
  for (let attempt = 1; attempt <= MAX_ASSESSMENT_REQUESTS; attempt += 1) {
    const response = UrlFetchApp.fetch(endpoint, {
      method: 'post', contentType: 'application/json', headers: { 'x-commsiq-ingest-secret': secret },
      payload: JSON.stringify({ batchId }), muteHttpExceptions: true
    });
    const code = response.getResponseCode();
    lastBody = response.getContentText();
    if (code < 200 || code >= 300) {
      console.error(`CommsIQ AI assessment failed for ${messageId} batch ${batchId}: HTTP ${code} ${lastBody}`);
      return { success: false, body: lastBody };
    }
    let result;
    try { result = JSON.parse(lastBody); }
    catch (error) {
      console.error(`CommsIQ assessment returned invalid JSON for ${messageId}`, error);
      return { success: false, body: lastBody };
    }
    console.log(`CommsIQ assessment progress for ${messageId} batch ${batchId}: ${lastBody}`);
    if (result.complete === true || result.alreadyAssessed === true || result.superseded === true) return { success: true, body: lastBody };
  }
  console.error(`CommsIQ assessment did not complete within ${MAX_ASSESSMENT_REQUESTS} requests for ${messageId} batch ${batchId}.`);
  return { success: false, body: lastBody };
}

function setupCommsIqTrigger() {
  for (const trigger of ScriptApp.getProjectTriggers()) if (trigger.getHandlerFunction() === 'ingestMotoSnapReports') ScriptApp.deleteTrigger(trigger);
  ScriptApp.newTrigger('ingestMotoSnapReports').timeBased().everyMinutes(5).create();
  ingestMotoSnapReports();
}

function resetCommsIqProcessedIds() { PropertiesService.getScriptProperties().deleteProperty(PROCESSED_KEY); }
...
function writeProcessedIds_(props, ids) { props.setProperty(PROCESSED_KEY, JSON.stringify(ids.slice(-MAX_PROCESSED_IDS))); }
