...
export function AuthGate({ children }: { children: ReactNode }) {
  ...
  async function verifyCode(event: FormEvent) {
    ...
    if (token.length < 6) return setError('Enter the full sign-in code.');
    const supabase = getSupabaseBrowserClient();
  ...
  async function verifyMfa(event: FormEvent) {
    ...
    if (token.length !== 6) return setError('Enter the full six-digit authenticator code.');
    const supabase = getSupabaseBrowserClient();
    ...
    <main className="flex min-h-screen items-center justify-center bg-[#f6f7f9] px-4 py-7 text-[#172033]">
      <section className="w-full max-w-[404px] overflow-hidden rounded-[13px] border border-[#cdd5df] bg-white shadow-[0_20px_44px_rgba(16,24,40,0.10)]">
        ...
        <div className="flex h-[148px] items-center justify-center bg-white px-7 pb-3 pt-7">
          <div className="relative h-[104px] w-[220px] max-w-[72%]"><Image src="/emich-automotive.png" alt="Emich Automotive" fill priority sizes="220px" className="object-contain" /></div>
        </div>
        <div className="px-7 pb-7">
          ...
          {stage === 'email' && <form onSubmit={requestCode}>
          ...
          {stage === 'code' && <form onSubmit={verifyCode}>
...
function RestoreScreen() {
  return <main className="flex min-h-screen items-center justify-center bg-[#f6f7f9] px-4 py-7 text-[#172033]">
...
function FieldLabel({ htmlFor, children }: { htmlFor: string; children: ReactNode }) { return <label htmlFor={htmlFor} className="mb-1.5 block text-[11px] font-bold uppercase tracking-[0.10em] text-[#344054]">{children}</label>; }
...
function PrimaryButton({ busy, children }: { busy: boolean; children: ReactNode }) { return <button type="submit" disabled={busy} className="min-h-10 w-full rounded-lg bg-[#1f4677] px-4 py-2.5 text-[13px] font-bold text-white transition hover:bg-[#173b67] disabled:opacity-60">{children}</button>; }
