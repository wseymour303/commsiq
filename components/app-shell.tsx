...
export function AppShell() {
  ...
  return <div className="min-h-screen">
    <header className="sticky top-0 z-30 border-b border-[var(--border)] bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/85"><div className="mx-auto flex h-[62px] max-w-[1480px] items-center justify-between px-3 sm:px-5 lg:px-7"><Brand/><nav className="hidden items-center gap-1 md:flex">{nav.map(n=><button key={n.id} onClick={()=>setActive(n.id)} className={`min-h-10 rounded-lg px-3 text-sm font-medium transition ${active===n.id?'bg-[var(--surface-subtle)] text-[var(--text)]':'text-[var(--muted)] hover:text-[var(--text)]'}`}>{n.label}</button>)}</nav><div className="flex h-9 w-9 items-center justify-center rounded-full border border-[var(--border)] bg-[var(--surface-subtle)] text-xs font-semibold">WS</div></div></header>
    <main>{active==='radar'?<ManagerRadar/>:<Placeholder active={active}/>}</main>
